...
class OCRController:
    ...
    def scan_document(self, pdf_path: Path, provider, config: ScanConfig, on_progress=None) -> OCRDocumentResult:
        ...
        for page_index in range(total):
            image = self.renderer.render_page_png(pdf_path, page_index, dpi=config.dpi)
            page_result = provider.scan_page(image, page_index=page_index, language_hint=config.language_hint)
